...
def _form_active_checks_dnos_diffcfg():
    return Dictionary(
        elements={
            'host': DictElement(
...
rule_spec_crl_url = ActiveCheck(
    title=Title('Check diff of running and rendered config'),
    help_text=Help('Check if rendered config of a Switch differs from the running config.'),
    topic=Topic.NETWORKING,
    name='dnos_diffcfg',
    parameter_form=_form_active_checks_dnos_diffcfg,
)
